...
void btbuildempty(Relation index) {
  ...
  smgrwrite(index->rd_smgr, INIT_FORKNUM, BTREE_METAPAGE, (char *)metapage,
            true);
 ...
 */
Size btestimateparallelscan(void) { return sizeof(BTParallelScanDescData); }
 ...
 *		page.  Other scans must wait until we call bt_parallel_release()
 *or bt_parallel_done().
 *
...
static bool _bt_vacuum_needs_cleanup(IndexVacuumInfo *info) {
  ...
  if (metad->btm_version < BTREE_VERSION) {
    ...
    cleanup_scale_factor =
        (relopts && relopts->vacuum_cleanup_index_scale_factor >= 0)
            ? relopts->vacuum_cleanup_index_scale_factor
            : vacuum_cleanup_index_scale_factor;
    prev_num_heap_tuples = metad->btm_last_cleanup_num_heap_tuples;

    if (cleanup_scale_factor <= 0 || prev_num_heap_tuples <= 0 ||
        (info->num_heap_tuples - prev_num_heap_tuples) / prev_num_heap_tuples >=
            cleanup_scale_factor)
      result = true;
  }
...
static void btvacuumscan(IndexVacuumInfo *info, IndexBulkDeleteResult *stats,
  ...
  vstate.pagedelcontext = AllocSetContextCreate(
      CurrentMemoryContext, "_bt_pagedel", ALLOCSET_DEFAULT_SIZES);
...
restart:
   ...
   */
  buf =
      ReadBufferExtended(rel, MAIN_FORKNUM, blkno, RBM_NORMAL, info->strategy);
  ...
  if (_bt_page_recyclable(page)) {
    ...
    if (callback) {
      for (offnum = minoff; offnum <= maxoff;
 ...
 */
bool btcanreturn(Relation index, int attno) { return true; }
